    ...
    def receiveGPSData_LR(self, initialState: np.array):
        ...
        while self.running is True:
            ...
            speed = (moveDirection * np.linalg.norm(
                pos - self.stateHistory[-1][:2])) / self.filter_dt
...
class MotorControlHandler:
    ...
    def toggleInPlace(self):
        if self.motor_running is False:
            self.motor_running = True
            inPlaceThread = threading.Thread(target=self.inPlaceRoutine, args=(
            3,))  # в args вводится периодичность с которой включается мотор, args =(2,) значит каждые 2 сек.
            inPlaceThread.start()
    ...
    def inPlaceRoutine(self, refreshTime):
        if not self.motor_running:
            return
        while self.motor_running:
            # mainmotor.motor_go(True, "Full", 600, 0.0005, False, 0.0000)
